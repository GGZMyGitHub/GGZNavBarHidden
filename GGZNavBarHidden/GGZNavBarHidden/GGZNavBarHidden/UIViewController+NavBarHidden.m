...
@property (nonatomic,assign) GGZHidenControlOptions  ggz_hidenControlOptions;// 设置导航条上的标签是否需要跟随滚动变化透明度,默认不会跟随滚动变化透明度
...
//定义关联的Key
static const char * ggz_hidenControlOptionsKey = "ggz_hidenControlOptions";
- (NSInteger)ggz_hidenControlOptions{
    
    return [objc_getAssociatedObject(self,ggz_hidenControlOptionsKey) integerValue];
}
- (void)setggz_hidenControlOptions:(NSInteger)ggz_hidenControlOptions{
    
    objc_setAssociatedObject(self, ggz_hidenControlOptionsKey, @(ggz_hidenControlOptions), OBJC_ASSOCIATION_ASSIGN);
}
...
- (void)setKeyScrollView:(UIScrollView *)keyScrollView scrolOffsetY:(CGFloat)scrolOffsetY options:(GGZHidenControlOptions)options{
  ...
  //  self.ggz_hidenControlOptions = options;
...
- (void)setNavSubViewsAlpha {
    ...
    self.navigationItem.leftBarButtonItem.customView.alpha = self.ggz_hidenControlOptions & 1?self.alpha:1;
    self.navigationItem.titleView.alpha = self.ggz_hidenControlOptions >> 1 & 1 ?self.alpha:1;
    self.navigationItem.rightBarButtonItem.customView.alpha = self.ggz_hidenControlOptions >> 2 & 1?self.alpha:1;
